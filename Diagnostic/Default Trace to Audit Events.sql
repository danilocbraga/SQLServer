DECLARE @FileName VARCHAR( MAX) SELECT @FileName = SUBSTRING(path , 0, LEN(path )-CHARINDEX( '\', REVERSE(path ))+1) + '\Log.trc'
FROM sys .traces
WHERE is_default = 1 ;
...
SELECT  TE . name ,
        ...
        ISNULL(T .NTDomainName ,'' ) as NTDomainName,
        ISNULL(t .NTUserName ,'' ) as NTUserName ,
        t . HostName ,
        t . ApplicationName ,
        t . LoginName ,
        --t .Duration ,
        ...
        CASE t . ObjectType
          ...
          ELSE 'Hmmm???'
        END AS ObjectType, ISNULL( TargetLoginName , '') as TargetLoginName ,ISNULL ( TextData,'' ) as TextData
FROM sys .fn_trace_gettable( @FileName, DEFAULT ) AS T
        JOIN sys .trace_events TE ON T . EventClass = TE . trace_event_id
        JOIN sys .trace_subclass_values v ON v . trace_event_id = TE .trace_event_id
                                            AND v . subclass_value = t .EventSubClass
WHERE   TE .name NOT IN ( 'Hash Warning', 'Sort Warnings','Audit Backup/Restore Event')
                -- filter statistics created by SQL server                                      
      -- AND t.ObjectType NOT IN ( 21587,22601 )-- filter tempdb objects
         AND DatabaseID <> 2
      -- AND NTUserName <> ''
--AND DB_NAME (t .DatabaseId ) = ''
